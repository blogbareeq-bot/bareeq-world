...
if (automated.transcriptionProvider !== 'Google Gemini API' || automated.transcriptionModel !== 'gemini-3.6-flash' || automated.transcriptionPassesPerPart !== 2) throw new Error('The Gemini 3.6 automated transcription contract is not the locked two-pass contract.');
...
if (report.status !== 'passed' || report.articleId !== ARTICLE_ID || report.audioMode !== 'six-segment-pilot' || report.partCount !== 1 || report.wordErrorCountAcrossAllPasses !== 0 || report.transcriptionModel !== 'gemini-3.6-flash') throw new Error('Gemini 3.6 automated transcript report identity/result mismatch.');
...
plan.audioReview = {
  ...
  evidence: {
    ...
    automatedTranscriptionModel: 'gemini-3.6-flash',
    wordErrorCount: 0,
  },
};
...
console.log(`Gemini listening evidence and two-pass Gemini 3.6 transcript evidence approved for ${ARTICLE_ID}; full synthesis is now allowed for this article only.`);
