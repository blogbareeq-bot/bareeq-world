...
const readinessAfter = readinessBefore
  .replace(
    ...
    "if (!['4.21.1', '4.21.2', '4.21.3', '4.21.4', '4.21.5'].includes(pkg.version)) failures.push(`Expected supported package version 4.21.1–4.21.4, got ${pkg.version}`);",
  )
  .replace(
  ...
  .replace(
    'Launch-readiness source audit passed: V4.21.3 package identity',
    'Launch-readiness source audit passed: V4.21.5 package identity',
  );
if (!readinessAfter.includes("['4.21.1', '4.21.2', '4.21.3', '4.21.4', '4.21.5'].includes(pkg.version)")) {
