...
const ASR_MODELS = ['gemini-3.6-flash', 'gemini-3.5-flash'];
...
async function transcribe(model, bytes, partNumber) {
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/interactions', {
    ...
    headers: {
      ...
      'User-Agent': 'Bareeq-Gemini31-Full-Dual-ASR/1.0',
    },
    body: JSON.stringify({
      model,
      input: [
        { type: 'text', text: prompt },
        { type: 'audio', data: bytes.toString('base64'), mime_type: 'audio/mp3' },
      ],
      response_format: {
        type: 'text',
  ...
  if (!response.ok) throw new Error(`${model} ASR part ${partNumber} failed (${response.status}): ${body.slice(0, 900)}`);
  let transcript;
  try {
    transcript = JSON.parse(extractGeminiOutputText(JSON.parse(body)).trim()).transcript?.trim();
  } catch (error) {
    throw new Error(`${model} ASR part ${partNumber} returned invalid structured transcript: ${error.message}`);
  }
  if (!transcript) throw new Error(`${model} ASR part ${partNumber} returned an empty transcript.`);
  return transcript;
}
...
const listFile = `/tmp/bareeq-${ARTICLE_ID}-31-dual-concat.txt`;
...
for (const model of ASR_MODELS) {
  ...
  for (const { index, bytes, asset } of partFiles) {
    const transcript = await transcribe(model, bytes, index + 1);
    chunks.push({ part: index + 1, audioSha256: asset.sha256, transcript, transcriptSha256: sha256(transcript) });
    console.log(`${model} part ${index + 1}/${partFiles.length}=DONE`);
  }
  const transcript = chunks.map((chunk) => chunk.transcript).join(' ');
  const comparison = compareArabicTranscripts(expected, transcript);
  console.log(`${model} FULL: expected=${comparison.expectedWordCount} actual=${comparison.actualWordCount} errors=${comparison.wordErrorCount}`);
  passes.push({
    provider: 'Google Gemini API', model, promptSha256: sha256(prompt), transcript, transcriptSha256: sha256(transcript), normalizedTranscript: comparison.actualNormalized,
    normalizedTranscriptSha256: sha256(comparison.actualNormalized), exact: comparison.exact, expectedWordCount: comparison.expectedWordCount,
    actualWordCount: comparison.actualWordCount, wordErrorCount: comparison.wordErrorCount, substitutions: comparison.substitutions,
    deletions: comparison.deletions, insertions: comparison.insertions, differences: comparison.operations.slice(0, 200), chunks,
  });
}
const passed = passes.length === 2 && new Set(passes.map((pass) => pass.model)).size === 2 && passes.every((pass) => pass.exact && pass.wordErrorCount === 0 && pass.substitutions === 0 && pass.deletions === 0 && pass.insertions === 0);
const report = {
  schema: 'bareeq.whole-article-dual-model-transcript-verification.v1', status: passed ? 'passed' : 'failed', articleId: ARTICLE_ID,
  ttsProvider: 'Google Gemini API', ttsModel: TTS_MODEL, ttsVoice: TTS_VOICE, audioMode: 'assembled-complete-article', audioFile: fullAudioFile,
  audioSha256: sha256(fullAudio), audioBytes: fullAudio.length, durationSeconds: Number(assembledDurationSeconds.toFixed(3)), partCount: manifest.parts.length,
  expectedTranscriptSha256: sha256(expected), expectedWordCount: passes[0]?.expectedWordCount ?? 0, independentModels: 2,
  wordErrorCountAcrossAllPasses: passes.reduce((sum, pass) => sum + pass.wordErrorCount, 0), substitutions: passes.reduce((sum, pass) => sum + pass.substitutions, 0),
  deletions: passes.reduce((sum, pass) => sum + pass.deletions, 0), insertions: passes.reduce((sum, pass) => sum + pass.insertions, 0),
  expectedTextDisclosure: 'Both ASR models received each immutable audio part plus generic transcription instructions only. Expected text was used only after responses for exact comparison.',
  comparisonProfile: ARABIC_TRANSCRIPT_COMPARISON_PROFILE, passes, verifiedAt: new Date().toISOString(),
};
await mkdir(path.dirname(reportFile), { recursive: true });
await writeFile(reportFile, `${JSON.stringify(report, null, 2)}\n`);
if (!passed) throw new Error(`Whole-article dual-model exact gate rejected audio: ${JSON.stringify(passes.map((pass) => pass.differences.slice(0, 20)))}`);
const reportBytes = await readFile(reportFile);
manifest.automatedTranscriptReview = {
  status: 'passed', scope: 'assembled-complete-article', reportFile, reportSha256: sha256(reportBytes), transcriptionProvider: 'Google Gemini API',
  transcriptionModels: passes.map((pass) => pass.model), transcriptionPasses: 2, expectedWordCount: report.expectedWordCount,
  wordErrorCountAcrossAllPasses: 0, substitutions: 0, deletions: 0, insertions: 0, fullAudioSha256: report.audioSha256, reviewedAt: report.verifiedAt,
};
await writeFile(manifestFile, `${JSON.stringify(manifest, null, 2)}\n`);
console.log(`WHOLE_ARTICLE_ZERO_ERROR=PASS words=${report.expectedWordCount} models=${passes.map((p) => p.model).join('+')} parts=${report.partCount} duration=${report.durationSeconds}s`);
