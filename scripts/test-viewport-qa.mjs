 ...
 * Opens the production build through astro preview. There is deliberately
 * no JSDOM fallback: unavailable Chromium is a failure, not a visual PASS.
 */
...
try {
  ...
  throw new Error('Playwright + Chromium are required for viewport QA.');
}
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const story of stories) {
      ...
      try {
        ...
        const headerBox = await page.locator('.visual-story__header').boundingBox();
        const footerBox = await page.locator('.visual-story__footer').boundingBox();
        ...
        if (failures.length > startFailureCount) {
          try {
            await page.screenshot({ path: resolve(artifactDir, `${safeName(story.slug)}-${viewport.width}x${viewport.height}.png`), fullPage: true });
          } catch {}
        }
        await page.close();
      }
